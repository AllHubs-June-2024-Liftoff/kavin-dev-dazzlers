...
pip install scikit-learn np pandas scipy
...
mycursor.execute("SELECT apiid, star_rating FROM review JOIN movie ON (review.movie_id=movie.id)")
...
mycursor.execute("SELECT apiid FROM user_favourite_movies JOIN movie ON (user_favourite_movies.favorite_movie_id=movie.id)")
myresult = mycursor.fetchall()
...
def get_movie_recommendation(movie_id):
    ...
    if len(movie_list):
      ...
      for val in rec_movie_indices:
            ...
            recommend_frame.append({'Title':movie_info.iloc[idx]['title'].values[0],'Distance':val[1],'movieId':int(movie_idx)})
...
moviedock_input = pd.concat([moviedock_favorites, moviedock_user_scores])
print("-----------------")
...
for x in moviedock_input.movieId:
  print(get_movie_recommendation(x))
  i = 1
